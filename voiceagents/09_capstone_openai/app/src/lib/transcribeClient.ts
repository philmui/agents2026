...
//
// This deliberately uses a HAND-BUILT WebRTC handshake (not the SDK) so you can
// see exactly what the SDK hides. The only differences from
...
// POST the SDP offer to /v1/realtime/calls) is verified in API_FACTS.md.
// ---------------------------------------------------------------------------
...
export class TranscribeClient {
  ...
  async start(token: string) {
    ...
    //    the SAME channel name the SDK uses under the hood (API_FACTS.md).
    ...
    dc.addEventListener("open", () => {
      ...
      // audio settings live under session.audio.input (API_FACTS.md).
      const configure = {
        type: "session.update",
        session: {
          type: "transcription",
          audio: {
            input: {
              // PCM16 @ 24 kHz mono is the Realtime audio format (API_FACTS.md).
              format: { type: "audio/pcm", rate: 24000 },
              // The dedicated realtime transcription model (API_FACTS.md).
              transcription: {
    ...
    //    few event types; the exact strings are verified in API_FACTS.md.
    ...
    //       The Content-Type MUST be application/sdp (API_FACTS.md).
